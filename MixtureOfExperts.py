...
class MOE(nn.Module):
    def __init__(self, num_experts, expert_size, k, bias_param, embed_size, bn, pn):
        super().__init__()
    ...
    def forward(self, x):
        ...
        probabilities = torch.sigmoid(routed)
        if self.training:
            _, k_idx = torch.topk((probabilities + torch.randn_like(probabilities) + self.bias), k=self.k, dim=1)
        else:
            _, k_idx = torch.topk((probabilities + self.bias), k=self.k, dim=1)
        k_probs = torch.gather(probabilities, dim=1, index=k_idx)
        k_probs = k_probs / (torch.sum(k_probs, dim=1).unsqueeze(dim=1))

        # Auxiliary Loss-Free Load Balancing -> adjust bias depending on amount of patches given to each expert
        if self.training:
            with torch.no_grad():
                update_bias = torch.bincount(k_idx.view(-1), minlength=self.num_experts)
                self.bias.add_(self.bias_param * (torch.sign(self.avg_load - update_bias)))

        # For each expert, calculate on group of patches that expert was selected for
